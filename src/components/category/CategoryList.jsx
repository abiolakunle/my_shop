  ...
  return (
    <div>
      ...
      >
        <List dense={true}>
          {!data.categories &&
            !status?.requesting.categories &&
            "No categories to display, are you offline?"}
          {!status?.requested.categories && <CircularProgress />}
          {status?.requested.categories &&
            ordered.categories.map((item, index) => {
              return (
                <Fragment key={index}>
                  <ListItem>
                    <ListItemAvatar>
